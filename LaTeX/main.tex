...
We evaluate the same seven-model matrix on two 72-case correctness datasets (\texttt{rigorous-temporal} and \texttt{rigorous-robustness}) and two latency-stability datasets, one per correctness suite (three repeated runs each, 216 total executions per model). The temporal set emphasizes explicit temporal grounding; the robustness set introduces perturbed and edge-case query forms. All runs use the same prompt contract, decoding settings, and CUDA local backend configuration where applicable.

All numeric values in this section are taken directly from the benchmark run artifacts: \texttt{report\_data/correctness/temporal/summary.json}, \texttt{report\_data/correctness/robustness/summary.json}, \texttt{report\_data/latency/temporal/summary.json}, and \texttt{report\_data/latency/robustness/summary.json}, together with the corresponding \texttt{gate\_results.json} files. End-to-end retrieval metrics (top-$N$ event hit rate, MRR) are recorded in the run artifacts but are all zero across both datasets and all models, because the events corpus was not available in the evaluation environment; only extraction metrics are reported here.
...
Table~\ref{table:latency_stability} reports the latency-stability suite across both \texttt{rigorous-temporal} and \texttt{rigorous-robustness}, each conducted with three runs per query (216 total executions per model per suite). The reported p95, standard deviations, and error rates are aggregated across all three runs. All seven models pass the latency-stability gate on both suites.

\begin{table*}[t]
    \centering
    \caption{Latency-Stability Suite ($72 \times 3 = 216$ executions per model per dataset). Gate thresholds: p95 $\leq 15{,}000$\,ms; timeout rate $\leq 0.10$; error rate $\leq 0.25$; parse stddev $\leq 0.10$; KW F1 stddev $\leq 0.10$; p95 stddev $\leq 4{,}000$\,ms. All models pass both suites.}
    \begin{tabular}{lcrrrrrr}
        \toprule
        \textbf{Model} & \textbf{Params} & \textbf{p95 (ms)} & \textbf{p95 Std (ms)} & \textbf{Parse Std} & \textbf{KW F1 Std} & \textbf{Error Rate} & \textbf{Gate} \\
        \midrule
        \multicolumn{8}{l}{\textit{(a) Temporal Dataset}} \\
        \midrule
        \texttt{gemma-3-27b-it} (Gemini)    & 27B   & 4{,}993 & 47.4  & 0.000 & 0.001 & 0.000 & Pass \\
        \texttt{llama3:latest} (Ollama)     & 8B    & 3{,}877 & 54.4  & 0.000 & 0.004 & 0.000 & Pass \\
        \texttt{Qwen2.5-1.5B}               & 1.5B  & 3{,}465 & 25.8  & 0.000 & 0.000 & 0.000 & Pass \\
        \texttt{TinyLlama-1.1B-Chat}        & 1.1B  & 2{,}187 & 26.3  & 0.000 & 0.000 & 0.014 & Pass \\
        \texttt{Qwen2.5-0.5B}               & 500M  & 2{,}859 & 33.0  & 0.000 & 0.000 & 0.014 & Pass \\
        \texttt{google/flan-t5-base}        & 250M  & 1{,}085 & 28.7  & 0.000 & 0.000 & 0.139 & Pass \\
        \texttt{LaMini-Flan-T5-248M}        & 248M  & 1{,}015 & 19.7  & 0.000 & 0.000 & 0.014 & Pass \\
        \midrule
        \multicolumn{8}{l}{\textit{(b) Robustness Dataset}} \\
        \midrule
...
Across both suites the variance profiles are consistently low. Qwen2.5-1.5B exhibits the tightest p95 standard deviation among local HuggingFace models on both temporal (25.8\,ms) and robustness (110.8\,ms), with zero parse and keyword F1 variance in each case. Gemini is similarly stable, with p95 stddev of 47.4\,ms and 36.8\,ms on temporal and robustness respectively, and zero parse variance---consistent with a deterministic remote API at temperature zero. Ollama records one timeout across 216 robustness executions (error rate 0.5\%) but runs cleanly on temporal (error rate 0.0\%), with minor keyword F1 variance (stddev 0.004) on both suites.

The most notable cross-suite difference is in error rate for the encoder-decoder models. On robustness, flan-t5-base records a 6.9\% error rate; on temporal this rises to 13.9\%, suggesting that the more complex temporal phrasing of \texttt{rigorous-temporal} places greater strain on the seq2seq architecture despite its low per-token compute cost. TinyLlama-1.1B and Qwen2.5-0.5B each introduce a 1.4\% error rate on temporal (zero on robustness for both), consistent with the occasional extreme-latency outliers observed in the single-run correctness data; across three runs the p95 values remain stable (2,187\,ms and 2,859\,ms respectively), confirming these as rare outlier events rather than systematic degradation.
...
\textbf{Determination: All models pass the latency-stability gate on both suites; stability metrics do not correlate monotonically with parameter count, and cross-suite variance profiles are broadly consistent.}

The latency-stability suite provides direct cross-run variance measurements for both \texttt{rigorous-temporal} and \texttt{rigorous-robustness}. All seven models satisfy every threshold of the latency-stability gate on both suites, indicating that none exhibit catastrophic instability under three repeated runs. Qwen2.5-1.5B shows the most consistent behavior among local models: zero variance in parse success and keyword F1 on both suites, with p95 standard deviations of 25.8\,ms (temporal) and 110.8\,ms (robustness). Gemini achieves near-zero variance across all stability metrics on both suites (p95 stddev 47.4\,ms temporal; 36.8\,ms robustness), consistent with a deterministic remote API at temperature zero.

The principal cross-suite differences involve error rates rather than latency variance. Ollama records one timeout on the robustness suite (error rate 0.5\%) but runs without error on temporal, suggesting that perturbed query forms are marginally harder for its inference path. TinyLlama-1.1B and Qwen2.5-0.5B each produce a 1.4\% error rate on temporal (zero on robustness), corresponding to rare extreme-latency outlier queries; the p95 values across three runs (2,187\,ms and 2,859\,ms respectively) confirm these are isolated events rather than systematic degradation. The most pronounced cross-suite shift is in flan-t5-base's error rate, which doubles from 6.9\% on robustness to 13.9\% on temporal---consistent with the seq2seq architecture being sensitive to the more complex temporal phrasing in \texttt{rigorous-temporal}---yet remains within the 25\% gate threshold. Stability, like correctness, therefore varies by model-dataset interaction rather than by parameter count alone.
...
\begin{figure}[ht]
    ...
    \caption{Latency-stability suite: latency distributions across three repeated runs on the robustness dataset (216 executions per model). The temporal suite exhibits an analogous distribution. The narrow p95 spread for most models reflects the low latency variance reported in Table~\ref{table:latency_stability}; T5-family models occupy the sub-1,100\,ms range while decoder-only models cluster between 1,300--5,000\,ms.}
