...
Figure~\ref{fig:jaccard} shows date/time extraction accuracy (fraction of 72 queries correct) relative to Gemini per model. Qwen 1.5B nearly matches Gemini at 66/72 (91.7\%); TinyLlama reaches 38/72 (53\%); Qwen 0.5B and Ollama llama3 fall to 14/72 (19\%) and 7/72 (10\%) respectively; T5 models score 2/72. Figure~\ref{fig:temporal} shows keyword search quality (mean result overlap, top-10) relative to Gemini, restricted to date-correct queries. All models achieve full overlap on date-correct queries, indicating that keyword extraction quality is not the differentiating factor --- the bottleneck is temporal grounding.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{../figures/correctness/temporal/fig1_jaccard.png}
    \caption{Date/time extraction accuracy (fraction of 72 queries correct) relative to Gemini. Qwen 1.5B nearly matches Gemini (66/72); TinyLlama reaches 53\%; Qwen 0.5B and Ollama llama3 fall to 19\% and 10\%; T5 models score near zero.}
    \label{fig:jaccard}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{Keyword search quality (mean result overlap, top-10) relative to Gemini baseline, restricted to date-correct queries only (events corpus unavailable for full evaluation). All models achieve full parity with Gemini on date-correct queries.}
    \label{fig:temporal}
\end{figure}

\subsection{Speed-Accuracy Frontier}

Figure~\ref{fig:quality_vs_speed} plots result overlap vs.\ Gemini (left axis) alongside p50 latency in seconds (right axis), with models sorted by quality. Gemini leads at 1.00 quality and 8.3\,s. Among local models, Qwen 1.5B is the strongest candidate at 0.97 quality and 5.7\,s; TinyLlama offers a favorable tradeoff at 0.83 quality and 2.5\,s. Qwen 0.5B achieves 0.82 quality at 3.6\,s --- comparable quality to TinyLlama but slower. Ollama llama3 lags sharply at 0.26 quality despite a similar 3.6\,s latency. T5 models are fastest (0.8\,s) but near-zero in quality (0.03).

\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{../figures/correctness/temporal/fig4_quality_vs_speed.png}
    \caption{Quality-latency tradeoff across models. Bars show result overlap vs.\ Gemini (left axis); line shows p50 latency in seconds (right axis). Qwen 1.5B is the best local model at 0.97 quality and 5.7\,s; TinyLlama offers 0.83 quality at 2.5\,s; Ollama llama3 lags at 0.26 quality despite similar latency.}
