...
Our experiments show asymmetric degradation across tasks and datasets. Temporal grounding is more fragile than keyword expansion under perturbation, and performance does not scale monotonically with parameter count alone. In particular, mid-scale local models (0.5B--1.5B) can remain usable under the correctness gate profile, while some very small and some endpoint-dependent configurations fail critical checks.
...
All numeric values in this section are taken directly from the synced artifacts under \texttt{figures/correctness/temporal/data/} and \texttt{figures/correctness/robustness/data/} (\texttt{summary.json} and \texttt{gate\_results.json}). Figures referenced below come from the corresponding temporal and robustness figure folders.

\subsection{Accuracy on the Temporal Dataset}

Table~\ref{table:temporal_accuracy} reports the canonical extraction metrics computed from the benchmark run artifacts. Parse success is the fraction of queries returning valid JSON. Date and time exact-match rates indicate strict matches against labeled temporal bounds; keyword F1 reports overlap between predicted and labeled keywords. All values are aggregated over the 72-query temporal benchmark and summarized from the run \texttt{summary.json}.
...
Table~\ref{table:robustness_accuracy} summarizes the same metrics on \texttt{rigorous-robustness}. This dataset has 48 date-labeled cases and 24 time-labeled cases in the run metadata. We report time \emph{partial} rate here because it is the criterion used by the correctness gates.

\begin{table*}[t]
    \centering
    \caption{Robustness Correctness Suite: Extraction Accuracy (72 queries, robustness dataset)}
    \begin{tabular}{lcccccr}
        \toprule
        ...
        \texttt{Qwen/Qwen2.5-1.5B}          & 1.5B  & 100\%   & 100\%   & 91.67\% & 0.0980 & 2{,}768 \\
        \texttt{TinyLlama-1.1B-Chat}        & 1.1B  & 100\%   & 100\%   & 95.83\% & 0.0478 & 1{,}412 \\
        \texttt{Qwen/Qwen2.5-0.5B}          & 500M  & 100\%   & 97.92\% & 62.50\% & 0.0507 & 1{,}932 \\
        \texttt{google/flan-t5-base}        & 250M  & 93.06\% & 0\%     & 0\%     & 0.000  & 858   \\
        \texttt{MBZUAI/LaMini-248M}         & 248M  & 98.61\% & 0\%     & 0\%     & 0.000  & 991   \\
        \bottomrule
    \end{tabular}
...
Using the correctness gates from \texttt{gate\_results.json}, temporal passes 4/7 models (Gemini, Qwen 1.5B, TinyLlama 1.1B, Qwen 0.5B), while robustness passes 2/7 (Qwen 1.5B and Qwen 0.5B). Both T5-family models (250M and 248M) fail both datasets with 0\% date exact, 0\% time performance, and 0 keyword F1, indicating non-usable extraction despite mostly parseable output. In robustness, Gemini also fails this run (25\% parse success), showing that usability depends on both model capacity and endpoint reliability under perturbation.
...
\subsection{Figure Cross-Checks}

\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{../figures/correctness/temporal/fig1_jaccard.png}
    \caption{Temporal-suite extraction correctness visualization from \texttt{figures/correctness/temporal/fig1\_jaccard.png}. The plotted ordering and relative separations align with Table~\ref{table:temporal_accuracy}: Gemini and Qwen 1.5B lead; T5-family models remain near zero.}
    \label{fig:jaccard}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{Temporal-suite keyword-overlap visualization from \texttt{figures/correctness/temporal/fig2\_temporal.png}. This chart uses the date-correct subset and should be interpreted jointly with keyword F1 in Table~\ref{table:temporal_accuracy}.}
    \label{fig:temporal}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{Temporal-suite quality-vs-speed plot from \texttt{figures/correctness/temporal/fig4\_quality\_vs\_speed.png}. Relative ranking is consistent with Table~\ref{table:temporal_accuracy}: Qwen 1.5B is the strongest local balance, while T5-family runs are fast but non-functional for temporal grounding.}
    \label{fig:quality_vs_speed}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{../figures/correctness/robustness/fig1_jaccard.png}
    \caption{Robustness-suite extraction correctness visualization from \texttt{figures/correctness/robustness/fig1\_jaccard.png}. The degradation pattern matches Table~\ref{table:robustness_accuracy}, including the non-usable T5-family outputs and stronger mid-scale local models.}
    \label{fig:robust_jaccard}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{../figures/correctness/robustness/fig4_quality_vs_speed.png}
    \caption{Robustness-suite quality-vs-speed plot from \texttt{figures/correctness/robustness/fig4\_quality\_vs\_speed.png}. The local trade-off frontier is led by Qwen 1.5B and Qwen 0.5B, consistent with robustness gate outcomes.}
    \label{fig:robust_quality_vs_speed}
\end{figure}

\section{Conclusion}
\label{sec:conclusion}

We present Curia-Lite, an efficiency study on model-size minimization for event query grounding. Across temporal and robustness suites, results confirm asymmetric degradation between keyword expansion and temporal grounding, plus meaningful backend sensitivity under perturbed queries. The best cross-suite local balance in our runs is Qwen 1.5B, while Qwen 0.5B remains a latency-oriented alternative that still passes robustness correctness gates.
